...
table_invalid_key_explanation = RPFM 检测到您的 Loc 文本中的某一行具有包含无效字符的键 (Key)。这可能会导致各种问题，包括崩溃，因此最好尽快修复它。造成这种情况的一个常见原因是旧版 PFM 代码（没错，就是 PFM）中的一个旧 Bug，如果您复制/粘贴 Loc 键，会导致它们在末尾添加无效字符。要修复它，请编辑报告的单元格并删除其上任何无效（通常是不可见）的字符。
