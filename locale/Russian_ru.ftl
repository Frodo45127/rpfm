...
table_invalid_key_explanation = Обнаружен некорректный символ в «ключе» записи { -loc(case: "genitive", capitalization: "lower", includeOriginalText: "true") }. Это может вызывать широкий спектр проблем, включая «вылеты» на рабочий стол. Настоятельно рекомендуется исправить такие записи как можно скорее.
