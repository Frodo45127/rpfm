...
pub struct AppUI {
    ...
    pub global_search_matches_close_button: *mut PushButton,
...
impl Default for AppUI {
    ...
    fn default() -> Self {
        ...
        // This corresponds to the flags `Movable` and `Floatable`.
        packfile_contents_dock_widget.set_features(Flags::from_int(6));
        ...
        let db_matches_frame = GroupBox::new(&QString::from_std_str("DB Matches"));
        let db_matches_grid = create_grid_layout_unsafe(db_matches_frame.as_mut_ptr() as *mut Widget);

        let loc_matches_frame = GroupBox::new(&QString::from_std_str("Loc Matches"));
        let loc_matches_grid = create_grid_layout_unsafe(loc_matches_frame.as_mut_ptr() as *mut Widget);

        // `TableView`s with all the matches.
        let close_matches_button = PushButton::new(&QString::from_std_str("Close Matches"));
        ...
        unsafe { global_search_dock_layout.as_mut().unwrap().add_widget((db_matches_frame.into_raw() as *mut Widget, 0, 0, 1, 1)); }
        unsafe { global_search_dock_layout.as_mut().unwrap().add_widget((loc_matches_frame.into_raw() as *mut Widget, 1, 0, 1, 1)); }
        unsafe { global_search_dock_layout.as_mut().unwrap().add_widget((close_matches_button.as_mut_ptr() as *mut Widget, 2, 0, 1, 1)); }

        // Action to update the search stuff when needed.
        let close_global_search_action = Action::new(());
        let update_global_search_stuff = Action::new(());
        ...
        AppUI {
            ...
            global_search_matches_close_button: close_matches_button.into_raw(),
