            ...
            let apiresponse = 
                if let Ok(mut remote_versions) = reqwest::get("https://raw.githubusercontent.com/Frodo45127/rpfm/master/schemas/versions.json") {
                    if let Ok(remote_versions) = remote_versions.json() {
                        ...
                        let local_versions: Versions = serde_json::from_reader(BufReader::new(File::open(get_config_path().unwrap().join(PathBuf::from("schemas/versions.json"))).unwrap())).unwrap();
