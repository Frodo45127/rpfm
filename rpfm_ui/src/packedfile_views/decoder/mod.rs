...
impl PackedFileDecoderView {
    ...
    pub unsafe fn new_view(
        ...
        let container_path = ContainerPath::File(file_view.path_copy());
        ...
        let hex_view_group = QGroupBox::from_q_string_q_widget(&QString::from_std_str("PackedFile's Data"), file_view.main_widget());
        ...
        let table_view = QTreeView::new_1a(file_view.main_widget());
        ...
        let decoded_fields_frame = QGroupBox::from_q_string_q_widget(&QString::from_std_str("Current Field Decoded"), file_view.main_widget());
        ...
        let info_frame = QGroupBox::from_q_string_q_widget(&QString::from_std_str("PackedFile Info"), file_view.main_widget());
        ...
        let table_view_old_versions = QTableView::new_1a(file_view.main_widget());
        ...
        let button_box = QFrame::new_1a(file_view.main_widget());
        ...
        file_view.view_type = ViewType::Internal(View::Decoder(packed_file_decoder_view));

        // Update the path so the decoder is identified as a separate file.
        let mut path = file_view.path_copy();
    ...
    pub fn import_from_assembly_kit(&self) -> Result<Vec<Vec<Field>>> {
        ...
        let max_steps = raw_definition.get_non_localisable_fields(&raw_localisable_fields.fields, &raw_table.rows[0]).len().checked_sub(1).unwrap_or_default();
