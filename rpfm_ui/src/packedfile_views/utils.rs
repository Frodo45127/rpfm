...
pub unsafe fn set_modified(is_modified: bool, path: &str, pack_key: &str, app_ui: &Rc<AppUI>, pack_file_contents_ui: &Rc<PackFileContentsUI>) {
    info!("Set Modified called for path: {path} (pack: {pack_key})");
    ...
    if is_modified {
        pack_file_contents_ui.packfile_contents_tree_view().update_treeview(true, TreeViewOperation::Modify(vec![path; 1]), DataSource::PackFile, pack_key);
        UI_STATE.set_is_modified(true, app_ui, pack_file_contents_ui);
    }
    else {
        pack_file_contents_ui.packfile_contents_tree_view().update_treeview(true, TreeViewOperation::Undo(vec![path; 1]), DataSource::PackFile, pack_key);
    }
}
