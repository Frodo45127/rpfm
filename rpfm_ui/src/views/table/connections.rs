...
pub unsafe fn set_connections(ui: &Arc<TableView>, slots: &TableViewSlots) {
    ...
    ui.get_mut_ptr_context_menu_copy_to_filter().triggered().connect(&slots.copy_to_filter);
    ...
    ui.get_mut_ptr_context_menu_find_references().triggered().connect(&slots.find_references);
