...
pub unsafe fn set_connections(ui: &Arc<TableView>, slots: &TableViewSlots) {
    ...
    ui.signal_mapper_profile_apply.mapped_string().connect(&slots.profile_apply);
    ui.signal_mapper_profile_delete.mapped_string().connect(&slots.profile_delete);
    ui.signal_mapper_profile_set_as_default.mapped_string().connect(&slots.profile_set_as_default);
