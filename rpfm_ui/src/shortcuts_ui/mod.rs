    ...
    pub unsafe fn load(shortcuts_ui: &Rc<Self>, shortcuts: &Shortcuts) {
        ...
        // Just add in mass the shortcuts to the Model, separated in sections.
        {
            ...
            for (key, value) in shortcuts.menu_bar_packfile.iter() {
                ...
                row_list.append_q_standard_item(&key.into_ptr().as_mut_raw_ptr());
                row_list.append_q_standard_item(&value.into_ptr().as_mut_raw_ptr());

                section.append_row_q_list_of_q_standard_item(row_list.as_ref());
            }

            menu_bar_packfile_parent.append_q_standard_item(&section.into_ptr().as_mut_raw_ptr());
            menu_bar_packfile_parent.append_q_standard_item(&fill1.into_ptr().as_mut_raw_ptr());
