        ...
        \".git\"
            ...
            Command::BuildCeoPost(pack_key, akit_path) => {
                ...
                let raw_bytes = match std::fs::read(&ceo_ccd_path) {
                    Ok(b) => b,
                    Err(e) => {
                        CentralCommand::send_back(&sender, Response::Error(format!("Failed to read ceo_data.ccd: {e}")));
                        continue 'background_loop;
                    }
                };
            ...
            Command::BuildCeoEntries(pack_key, entries) => {
                let result = (|| -> Result<Vec<ContainerPath>> {
                    ...
                    fn get_or_create_db<'a>(
                        pack: &'a mut rpfm_lib::files::pack::Pack,
                        schema: &rpfm_lib::schema::Schema,
                        table_name: &str,
                        file_stem: &str,
                    ) -> anyhow::Result<String> {
                        let path = format!("db/{}/{}", table_name, file_stem);
                        let container_path = rpfm_lib::files::ContainerPath::File(path.clone());
                        ...
                        if pack.files_by_paths(&[container_path.clone()], false).is_empty() {
                            ...
                            let db = rpfm_lib::files::db::DB::new(def, patches, &table_name_with_suffix);
                            let rfile = rpfm_lib::files::RFile::new_from_decoded(
                                &rpfm_lib::files::RFileDecoded::DB(db), 0, &path
                            );
                            pack.insert(rfile)?;
                        }
                        Ok(path)
                    ...
                    fn insert_row(
                        pack: &mut rpfm_lib::files::pack::Pack,
                        schema: &rpfm_lib::schema::Schema,
                        table_name: &str,
                        file_stem: &str,
                        values: &std::collections::HashMap<&str, String>,
                    ) -> anyhow::Result<ContainerPath> {
                        ...
                        let mut files = pack.files_by_type_and_paths_mut(
                            &[rpfm_lib::files::FileType::DB],
                            &[container_path.clone()],
                            true,
                        );

                        if let Some(file) = files.first_mut() {
                            if let Ok(rpfm_lib::files::RFileDecoded::DB(db)) = file.decoded_mut() {
                                let fields = db.definition().fields_processed().to_vec();
                                let mut row: Vec<rpfm_lib::files::table::DecodedData> = Vec::new();
                                ...
                                for field in &fields {
                                    ...
                                    let cell = match field.field_type() {
                                        rpfm_lib::schema::FieldType::Boolean =>
                                            rpfm_lib::files::table::DecodedData::Boolean(raw == "true" || raw == "1"),
                                        rpfm_lib::schema::FieldType::I16 =>
                                            rpfm_lib::files::table::DecodedData::I16(raw.parse().unwrap_or(0)),
                                        rpfm_lib::schema::FieldType::I32 =>
                                            rpfm_lib::files::table::DecodedData::I32(raw.parse().unwrap_or(0)),
                                        rpfm_lib::schema::FieldType::I64 =>
                                            rpfm_lib::files::table::DecodedData::I64(raw.parse().unwrap_or(0)),
                                        rpfm_lib::schema::FieldType::F32 =>
                                            rpfm_lib::files::table::DecodedData::F32(raw.parse().unwrap_or(0.0)),
                                        rpfm_lib::schema::FieldType::F64 =>
                                            rpfm_lib::files::table::DecodedData::F64(raw.parse().unwrap_or(0.0)),
                                        rpfm_lib::schema::FieldType::OptionalI16 =>
                                            rpfm_lib::files::table::DecodedData::OptionalI16(raw.parse().unwrap_or(0)),
                                        rpfm_lib::schema::FieldType::OptionalI32 =>
                                            rpfm_lib::files::table::DecodedData::OptionalI32(raw.parse().unwrap_or(0)),
                                        rpfm_lib::schema::FieldType::OptionalI64 =>
                                            rpfm_lib::files::table::DecodedData::OptionalI64(raw.parse().unwrap_or(0)),
                                        rpfm_lib::schema::FieldType::OptionalStringU8 =>
                                            rpfm_lib::files::table::DecodedData::OptionalStringU8(raw.to_string()),
                                        rpfm_lib::schema::FieldType::OptionalStringU16 =>
                                            rpfm_lib::files::table::DecodedData::OptionalStringU16(raw.to_string()),
                                        rpfm_lib::schema::FieldType::StringU16 =>
                                            rpfm_lib::files::table::DecodedData::StringU16(raw.to_string()),
                                        _ =>
                                            rpfm_lib::files::table::DecodedData::StringU8(raw.to_string()),
                                    };
                                    row.push(cell);
                                }
                                db.data_mut().push(row);
                    ...
                    fn insert_loc_entries(
                        ...
                        if pack.files_by_paths(&[container_path.clone()], false).is_empty() {
                            let loc = rpfm_lib::files::loc::Loc::new();
                            let rfile = rpfm_lib::files::RFile::new_from_decoded(
                                &rpfm_lib::files::RFileDecoded::Loc(loc), 0, loc_path
                            );
                            pack.insert(rfile)?;
                        }

                        let mut files = pack.files_by_type_and_paths_mut(
                            &[rpfm_lib::files::FileType::Loc],
                            &[container_path.clone()],
                            true,
                        );

                        if let Some(file) = files.first_mut() {
                            if let Ok(rpfm_lib::files::RFileDecoded::Loc(loc)) = file.decoded_mut() {
                                for (key, text) in entries {
                                    loc.data_mut().push(vec![
                                        rpfm_lib::files::table::DecodedData::StringU16(key.to_string()),
                                        rpfm_lib::files::table::DecodedData::StringU16(text.to_string()),
                                        rpfm_lib::files::table::DecodedData::Boolean(true),
                                    ]);
                                }
                            }
                        }
