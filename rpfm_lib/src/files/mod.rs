                    ...
                    if rfile.file_type() == FileType::DB || rfile.file_type() == FileType::Loc {
                        ...
                        if result.is_err() {
                            ...
                            #[cfg(feature = "integration_log")] {
                                warn!("File with path {} failed to extract as TSV. Did you forget the --schema flag? Extracting it as binary.", rfile.path_in_container_raw());
                            }
            ...
            Some(extension) => {
                if extension.to_string_lossy() == "tsv" {
                    ...
                    if let Err(_error) = rfile {
                        ...
                        #[cfg(feature = "integration_log")] {
                            warn!("File with path {} failed to import as TSV. Did you forget the --schema flag? Importing it as binary. Error was: {}", &source_path.to_string_lossy(), _error);
                        }
                ...
                Some(extension) => {
                    if extension.to_string_lossy() == "tsv" {
                        ...
                        if let Err(_error) = rfile {
                            ...
                            #[cfg(feature = "integration_log")] {
                                warn!("File with path {} failed to import as TSV. Did you forget the --schema flag? Importing it as binary. Error was: {}", &file_path.to_string_lossy(), _error);
                            }
