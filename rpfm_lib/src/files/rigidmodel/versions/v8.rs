...
impl RigidModel {
    ...
    pub(crate) fn read_v8<R: ReadBytes>(&mut self, data: &mut R) -> Result<()> {
        ...
        for _ in 0..lod_count {
            ...
            lod.authored_lod_number = data.read_u32()?;
            lod.quality_level = data.read_u32()?;
        ...
        for lod in &mut self.lods {
            for _ in 0..lod.mesh_blocks.capacity() {
                ...
                //mesh.mesh.shader_params.name = data.read_string_u8_0padded(PADDED_SIZE_12)?;
    ...
    pub(crate) fn write_v8<W: WriteBytes>(&self, buffer: &mut W) -> Result<()> {
        ...
        for lod in self.lods() {
            ...
            for mesh in lod.mesh_blocks() {
                ...
                //mesh_data.write_string_u8_0padded(mesh.mesh.shader_params().name(), PADDED_SIZE_12, true)?;
            ...
            // 7 items per lod header * 4 bytes per item * X num of lods + 134 (header lenght) + data until current lod.
            lod_headers.write_u32((7 * 4 * self.lods().len() as u32) + HEADER_LENGTH + offset_current_lod)?;
            offset_current_lod = lod_data.len() as u32;

            lod_headers.write_f32(lod.visibility_distance)?;
            lod_headers.write_u32(lod.authored_lod_number)?;
            lod_headers.write_u32(lod.quality_level)?;
        }
