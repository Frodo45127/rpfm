...
impl Pack {
    ...
    pub(crate) fn read_pfh2<R: ReadBytes>(&mut self, data: &mut R, extra_data: &DecodeableExtraData) -> Result<u64> {
    ...
    pub(crate) fn write_pfh2<W: WriteBytes>(&mut self, buffer: &mut W, extra_data: &Option<EncodeableExtraData>) -> Result<()> {
