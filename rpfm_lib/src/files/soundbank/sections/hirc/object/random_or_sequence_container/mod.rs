...
pub struct RandomOrSequenceContainer {
    data: Vec<u8>,
}
...
impl RandomOrSequenceContainer {

    pub(crate) fn read<R: ReadBytes>(data: &mut R, version: u32, size: usize) -> Result<Self> {
        match version {
            122 => Self::read_v122(data, size),
            _ => Err(RLibError::SoundBankUnsupportedVersionFound(version, "RandomOrSequenceContainer".to_string())),
        }
    }
