...
const TIPS_REPO: &str = "https://github.com/Frodo45127/rpfm-messages";
...
/// Individual tip.
#[derive(Clone, PartialEq, Eq, Debug, Serialize, Deserialize, GetRef, Set)]
pub struct Tip {
...
impl Tips {
    ...
    pub fn load() -> Result<Self> {
        let mut local_file_path = get_config_path()?.join(TIPS_LOCAL_FOLDER);
        let mut remote_file_path = get_config_path()?.join(TIPS_REMOTE_FOLDER);
        local_file_path.push(TIPS_LOCAL_FILE);
        remote_file_path.push(TIPS_REMOTE_FILE);

        let local_tips = if let Ok(tipset) = TipSet::load(&local_file_path) { tipset } else { TipSet::default() };
        let remote_tips = if let Ok(tipset) = TipSet::load(&remote_file_path) { tipset } else { TipSet::default() };
    ...
    pub fn get_local_tips_by_id(&self, id: u64) -> Option<&Tip> {
        let game_key = GAME_SELECTED.read().unwrap().get_game_key_name();
        if let Some(game_tips) = self.local_tips.tips.get(&game_key) {
            game_tips.iter()
                .find_map(|(_, tips)| tips.iter().find(|tip| tip.get_ref_id() == &id))
        } else {
            None
        }
    }
    ...
    pub fn get_local_tips_for_path(&self, path: &[String]) -> Vec<Tip> {
        let game_key = GAME_SELECTED.read().unwrap().get_game_key_name();
        if let Some(game_tips) = self.local_tips.tips.get(&game_key) {
            let path = path.join("/");
            game_tips.iter()
                .filter(|(tip_path, _)| tip_path.starts_with(&path))
                .map(|(_, tips)| tips.to_vec())
                .flatten()
                .collect()
        } else {
            vec![]
    ...
    pub fn get_remote_tips_for_path(&self, path: &[String]) -> Vec<Tip> {
        let game_key = GAME_SELECTED.read().unwrap().get_game_key_name();
    ...
    pub fn add_tip_to_local_tips(&mut self, tip: Tip) {
        let game_key = GAME_SELECTED.read().unwrap().get_game_key_name();

        // First, delete any tip with the same id, so in case of editing, we "overwrite" the old one instead of creating a new one.
        self.delete_tip_by_id(*tip.get_ref_id());

        // Then, add the new tip.
        match self.local_tips.tips.get_mut(&game_key) {
            Some(game_tips) => match game_tips.get_mut(tip.get_ref_path()) {
                Some(tips) => tips.push(tip),
                None => { game_tips.insert(tip.get_ref_path().to_owned(), vec![tip]); }
            }
            None => {
                let mut game_tips = HashMap::new();
                game_tips.insert(tip.get_ref_path().to_owned(), vec![tip]);
    ...
    pub fn add_tip_to_remote_tips(&mut self, tip: Tip, save: bool) -> Result<()> {
        let game_key = GAME_SELECTED.read().unwrap().get_game_key_name();
        ...
        if save {
            let mut file_path = get_config_path()?.join(TIPS_REMOTE_FOLDER);

            // Make sure the path exists to avoid problems with updating schemas.
            DirBuilder::new().recursive(true).create(&file_path)?;

            file_path.push(TIPS_REMOTE_FILE);
            let mut file = File::create(&file_path)?;
    ...
    pub fn delete_tip_by_id(&mut self, id: u64) {
        let game_key = GAME_SELECTED.read().unwrap().get_game_key_name();

        if let Some(old_game_tips) = self.local_tips.tips.get_mut(&game_key) {
            old_game_tips.iter_mut().for_each(|(_, tips)| tips.retain(|old_game_tip| old_game_tip.get_ref_id() != &id));
...
impl TipSet {
    ...
    pub fn load(file_path: &Path) -> Result<Self> {
        ...
        serde_json::from_reader(file).map_err(|error| Error::from(error))
    }
}
